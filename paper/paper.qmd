...
author: 
  - Amie Liu, Wanling Ma
thanks: "Code and data are available at: https://github.com/WanlingMa/The-Impact-of-Tax-Policy-Evolution-on-Economic-Development-and-Income-Inequality; This reproduction was performed after a replication on the Social Science Reproduction platform: https://www.socialsciencereproduction.org/reproductions/0527a4db-9300-4160-ae8c-656c449f6d24/index"
date: today
date-format: long
abstract: "The role of taxation in economic development is crucial, influencing wealth distribution and shaping economic activity regulation. This paper uses R programming language to replicate and analyze data to understand the evolution of tax policy and its impact on income inequality and economic development. Our findings reveal a significant shift towards direct taxation as economies advance. Further research is necessary to address policy implementation challenges and refine optimal tax policy frameworks supporting equitable economic development."
format: pdf
toc: true
...
In today's globalized economy, taxation's role in economic development is central to both academic inquiry and policymaking. Tax policies shape the distribution of wealth and opportunities within countries. It functions as a regulatory instrument when the government's objective is to encourage or restrain economic activity through the reduction or escalation of income tax and other taxes, as well as contributions that influence people’s disposable income and consumption [@taxpolicy]. As countries strive to achieve sustainable growth and equitably distribute resources, it is critical to understand how tax policy evolves and its impact on economic development and income inequality. 

“Tax Equity in Low- and Middle-Income Countries” by Pierre Bachas, Anders Jensen, and Lucie Gadenne investigates tax equity and its implications in low- and middle-income countries. They find a shift from indirect to personal income taxes as countries move towards higher levels of economic development [@article]. In low-income countries, a significantly higher percentage of revenue comes from indirect taxes (65%) compared to high-income countries (48%) [@article]. Moreover, the prevalence of the informal sector significantly affects the progressivity of indirect taxes like Value-Added Tax (VAT), contrary to common perception [@article]. The estimand which we aim to replicate encompasses the aforementioned findings and explores the relationship between the transition from indirect to direct taxation and economic growth in low- and middle-income countries. We seek to quantify how changes in tax policy influence key economic indicators such as GDP growth and employment. By examining this relationship, our paper provides valuable insights into the impact of tax policy evolution on overall economic development.

Despite the insightful exploration of tax equity in low- and middle-income countries by @article, there remains an underexplored area concerning the effectiveness of tax policy changes on the informal sector. While the shift from indirect to direct taxation is well-documented, its impact on the informal economy—a significant component of low- and middle-income countries—lacks a detailed analysis. Furthur studies should focus on evaluating how tax policy transitions influence the informal sector, thereby affecting overall economic development and equity.

This paper reproduces, replicates, and expands on these findings, using the R programming language [@citeR], the 'tidyverse' [@citeTidyverse] 'labelled' [@citelabelled], 'patchwork' [@citepatchwork], and 'haven' [@citehaven] packages. Within the 'tidyverse' package [@citeTidyverse], we used 'readr' [@citeReadr], 'dplyr' [@citeDplyr], 'tidyr' [@citeTidyr], and 'ggplot2' [@citeGgplot2]. It examines the implications of tax policy evolution on economic development and income inequality, which raises the research question: "How does the transition from indirect to direct taxation, alongside the dynamics of the informal sector, influence tax equity and economic development in low- and middle-income countries, and what are the implications for designing effective and equitable tax policies?" 

Our analysis reveals significant findings about the dynamics of tax policy in low- and middle-income countries and its relationship to economic development and equity. We observe a clear trend showing a transition from indirect to direct taxation as economies advance and reflecting a shift towards tax systems that are more progressive. This shift has the potential to reduce income inequality by placing a larger tax burden on higher-income households. Furthermore, contrary to conventional views, indirect taxes can exhibit progressive features in economies with substantial informal sectors. This challenges traditional tax policy narratives, emphasizing the need for specific tax policy designs. However, difficulties are present in effectively taxing high-income households, particularly in low- and middle-income countries, where issues of tax evasion and enforcement challenges are major barriers to the effectiveness of direct taxation on wealthy households. These difficulties are critical for understanding the limitations of current tax systems in achieving equity. Therefore, our findings underscore the complexity of tax policy evolution and its implications for income inequality and economic development in low- and middle-income countries.

This paper is structured into three main sections: Data, Results, and Discussion. The Data section (@sec-data) delves into sources and methodology. It explores the various sources used for data collection, cleaning, and analyzing processes, drawing from the dataset originally published by the American Economic Association [@aea] and accessed through the openICPSR Portal [@openicpsr]. In the Results section (@sec-results), we replicate and expand findings from the data of “Tax Equity in Low- and Middle-Income Countries” [@article], focusing on data visualizations and analyses. Discussion (@sec-discussion) provides a comprehensive analysis of the research findings, highlights the importance of ethical considerations in tax policy analysis, and identifies areas for future research.
...
This paper used the R programming language [@citeR] along with the 'tidyverse' package [@citeTidyverse], as well as the 'labelled' [@citelabelled], 'patchwork' [@citepatchwork], and 'haven' [@citehaven] packages for replication, data cleaning, and analysis. Within the 'tidyverse' package [@citeTidyverse], we used 'readr' [@citeReadr], 'dplyr' [@citeDplyr], 'tidyr' [@citeTidyr], and 'ggplot2' [@citeGgplot2] for reading CSV files, manipulating data, tidying data, and creating graphs.

Data collection methods exhibit significant diversity among open sources. To ensure the quality of our dataset, we conducted a data-cleaning procedure for reproduction, focusing on variable selection, simplifying variable names, and removing missing values. The variables selected from the original paper were chosen based on their availability and the relevance to our research question: How does the transition from indirect to direct taxation, alongside the dynamics of the informal sector, influence tax equity and economic development in low- and middle-income countries, and what are the implications for designing effective and equitable tax policies? 

In the research presented within this paper, we place a strong emphasis on the detailed analysis of variable measurements to ensure the validity and reliability of our findings. This process involved defining clear operational meanings for key concepts and committing to consistent measurement across the various variables examined. By analyzing datasets from multiple years and sources, such as the World Bank's World Development Indicators and the Harvard Dataverse for oil and natural gas production, we have ensured that our data remains consistent and reliable over time. This careful approach to measurement was further supported by robustness checks, confirming the stability of our data across different periods, thus enhancing the credibility of our analyses and conclusions. Such careful attention to the measurement process highlights our commitment to conducting a comprehensive and precise exploration of the topics at hand.
...
# Generate Figure 3 and 4, stored in plot1 and plot2
...
This observation aligns with global trends noted in academic literature. For instance, @genschel2016tax conducted a research on taxation trends outside the OECD and revealed that while non-Western countries adopt Western tax trends like increasing general revenues and implementing consumption taxes, they struggle with establishing a strong, progressive personal income tax system. This struggle is attributed to various socio-economic and structural factors, which, despite global influences, hinder the replication of progressive tax regimes seen in more developed economies.

Moreover, @stoilova2013empirical provide a comprehensive analysis on the impact of taxation on economic growth within the European Union, offering empirical evidence of how tax structures affect economic performance from 1995 to 2010. Their findings, alongside the examination of tax revenue trends and social contributions in European Union countries by @dubrovina2019nexus, underscore the significance of adapting tax policies to enhance fiscal roles and ensure equitable distribution without overburdening the population.

These studies corroborate the trends shown in the provided figures, reinforcing the global narrative of evolving tax structures as economies grow. They not only validate the empirical evidence from the study but also expand the understanding of how these trends manifest in different contexts, from OECD countries to emerging economies. Such analyses underscore the complexity and dynamism of tax policy evolution, reflecting a broad consensus on the direction of tax policy reforms aimed at fostering economic development and social equity.
...
Literature supports this potential, with studies highlighting the role of tax policy in addressing income inequality. @ballardrosa2017structure delve into the political economy aspects, suggesting that preferences over taxation significantly influence policy responses to income inequality. Moreover, @mozdzierz2015financialisation identifies financialization as a primary driver of increasing income inequality, indicating the need for tax policies that can counteract these trends effectively.

Further, research by Liu Qiongzhi highlights the decisive role of policy bias in determining fiscal redistribution policies, emphasizing the importance of favoring equitable tax rates and transfer payments to address income disparities [@liu2020policybias]. @khan2021taxexpenditures find that in South Asia, taxation and social expenditure policies effectively reduce income inequality, highlighting the effectiveness of direct taxes and higher social spending as tools for income redistribution.

The complexity and challenges in designing effective tax policies are underscored by outliers in data, suggesting that not all shifts towards direct taxation seamlessly lead to reduced income inequality. Factors such as tax evasion and the informal economy pose significant challenges. Studies like Esaku's demonstrate how income inequality can exacerbate the size of the shadow economy, further complicating tax policy design and implementation [@esaku2021incomeinequality]. Additionally, @wildowiczszumarska2022eu discusses the effectiveness of redistributive policies within the EU, indicating varied success across different welfare state models in tackling income inequality through tax policy.

## Informality and Taxation

The interplay between the informal sector and tax policy design is pivotal for economic frameworks, particularly in developing economies. @fig-informality-selfemployment and @fig-informality-consumption, showing a decline in informality with increased GDP per capita, underscores a critical transition phase for tax policy considerations. This visual evidence aligns with literature indicating that as economies grow, the informal sector's proportionate contribution to the economy tends to diminish, posing both challenges and opportunities for tax policy [@loayza2016informality]. Loayza's work explains how the shrinking of the informal sector as a result of economic development necessitates adaptable tax policies capable of accommodating this transition. Specifically, tax policy effectiveness hinges on recognizing the evolving tax base and ensuring that tax structures are responsive to the changing landscape of formal and informal economic activities.

The implications of these findings suggest that policymakers must prioritize the integration of the informal sector into formal economic considerations, tailoring tax policies to be more inclusive and reflective of an economy's developmental stage. This approach not only enhances tax revenue potential but also supports the formalization of informal activities, contributing to broader economic development goals.
